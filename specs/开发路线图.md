...
| 命令白名单：shell 命令审批三键（拒绝/允许一次/永远允许）+ 插件侧白名单自动放行 | P1 | 约 5 天（含 spike） | 无（复用 conversation composer chain） | ✅ 编码完成（W0 spike 通过：priority 0 接管 + overlay 回落 + 自建应答封装；58 断言全绿。真机验证待执行，见 `docs/开发记录/命令白名单-实现完成报告.md`） |
| 聊天内打开文件：消息流中 agent 编辑/引用的文件可直接在 DSH 窗口打开（复用文件树面板 openFile） | P1 | 约 2 天 | 文件树面板（openFile/setPanelOpen/setReveal） | ✅ 编码完成（拦截 workspaces.openPath：文件+工作区内→DSH，目录/工作区外→系统；66 断言全绿。真机验证待执行，见 `docs/开发记录/聊天内打开文件-实现完成报告.md`） |
